-- Script SQL để sửa lỗi tên sản phẩm và cập nhật đầy đủ danh mục sản phẩm
...
-- Sửa các tên sản phẩm bị lỗi chính tả hoặc không nhất quán
UPDATE school_supplies
...
UPDATE school_supplies
SET name = 'Thước kẻ 30cm Thiên Long'
WHERE name = 'Thước kẻ' OR name LIKE '%Thước kẻ 30%' OR name LIKE '%Thiên Long%';

UPDATE school_supplies
SET name = 'Compa Thiên Long C-019'
WHERE name = 'Compa' OR name LIKE '%Compa%' OR name LIKE '%Thiên Long C-019%';

UPDATE school_supplies
SET name = 'Hộp bút nhựa trong suốt'
WHERE name = 'Hộp bút' OR name LIKE '%Hộp bút nhựa%';

UPDATE school_supplies
SET name = 'Balo học sinh Hami BL001'
WHERE name = 'Balo học sinh' OR name LIKE '%Balo%Hami%';

UPDATE school_supplies
SET name = 'Vở ô ly 200 trang Campus NB-BSTY200'
WHERE name = 'Vở ô ly' OR name LIKE '%Vở ô ly%Campus%';

UPDATE school_supplies
SET name = 'Máy tính Casio FX-570VN Plus'
WHERE name LIKE '%Máy tính Casio%' OR id = 15;

UPDATE school_supplies
SET name = 'Bộ 10 viên phấn trắng'
WHERE name LIKE '%phấn%' OR id = 17;

UPDATE school_supplies
SET name = 'Bìa nhựa đựng tài liệu'
WHERE name LIKE '%Bìa nhựa%' OR name LIKE '%đựng tài liệu%' OR id = 31;

UPDATE school_supplies
SET name = 'Đèn bàn học LED'
WHERE name LIKE '%Đèn bàn học%' OR name LIKE '%đèn học%' OR id = 47;
...
WHERE name LIKE '%kẹp%' 
   ...
   OR name LIKE '%kẹp gỗ%'
   OR name LIKE '%Bìa nhựa%' 
   OR name LIKE '%đựng tài liệu%'
   OR id = 31;
...
WHERE name LIKE '%cặp%' 
   ...
   OR name LIKE '%Máy tính Casio%'
   OR name LIKE '%Đèn bàn học%' 
   OR name LIKE '%đèn học%'
   OR id = 15
   OR id = 47;
...
WHERE name LIKE '%cặp học sinh%' 
   OR name LIKE '%túi học sinh%' 
   OR name LIKE '%ba lô học sinh%' 
   OR name LIKE '%balo học sinh%' 
   OR name LIKE '%hộp cơm%' 
   OR name LIKE '%bình nước%' 
   OR name LIKE '%đồng phục%';

-- Cập nhật sản phẩm cho danh mục "Thiết bị Văn phòng"
UPDATE school_supplies 
SET category_id = (SELECT id FROM categories WHERE name = 'Thiết bị Văn phòng') 
WHERE name LIKE '%máy tính%' 
   OR name LIKE '%máy in%' 
   OR name LIKE '%máy photo%' 
   OR name LIKE '%máy scan%' 
   OR name LIKE '%máy hủy%' 
   OR name LIKE '%máy đếm%' 
   OR name LIKE '%máy chấm công%';
...
SELECT id, name FROM school_supplies WHERE category_id IS NULL OR category_id = 0;
...
-- Kiểm tra danh sách sản phẩm theo từng danh mục
SELECT c.name as category_name, s.id, s.name as product_name
FROM categories c
LEFT JOIN school_supplies s ON c.id = s.category_id
